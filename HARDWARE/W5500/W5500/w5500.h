...
void socket_buf_init(unsigned char * tx_size, unsigned char * rx_size); // setting tx/rx buf size
...
void setRTR(unsigned short timeout); // set retry duration for data transmission, connection, closing ...
void setRCR(unsigned char retry); // set retry count (above the value, assert timeout interrupt)
void clearIR(unsigned char mask); // clear interrupt
unsigned char getIR( void );
void setSn_MSS(SOCKET s, unsigned short Sn_MSSR); // set maximum segment size
unsigned char getSn_IR(SOCKET s); // get socket interrupt status
unsigned char getSn_SR(SOCKET s); // get socket status
unsigned short getSn_TX_FSR(SOCKET s); // get socket TX free buf size
unsigned short getSn_RX_RSR(SOCKET s); // get socket RX recv buf size
...
extern unsigned char txsize[];
extern unsigned char rxsize[];
extern void SET_W5500_MAC(void);
extern unsigned char PHY_Check(void);
extern void setDIPR(SOCKET s,unsigned char* buff);
extern void setDPORT(SOCKET s,unsigned short port);
/**
