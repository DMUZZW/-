...
*/
uint8 socket(SOCKET s, uint8 protocol, unsigned short port, uint8 flag)
{
   uint8 ret;
	unsigned short local_port=0;
		if (s == (SOCK_TCPC|SOCK_TCPS))
	{
		local_port =tcp_local_port;//如果是用TCP通信方式 ，则使用TCP端口号
	}
	else if(s ==SOCK_UDPS)
	{
		local_port = udp_local_port;//如果是采用UDP通信方式，则使用UDP端口号
	}
   if (
...
*@return  1 for success else 0.
*/
uint8 connect(SOCKET s, uint8 * addr, unsigned short port)
{
    uint8 ret;
    if
        (
...
*@return  1 for success else 0.
*/
unsigned short send(SOCKET s, const uint8 * buf, unsigned short len)
{
  uint8 status=0;
...
*/
unsigned short recv(SOCKET s, uint8 * buf, unsigned short len)
{
   unsigned short ret=0;
   if ( len > 0 )
   {
...
*/
unsigned short sendto(SOCKET s, const uint8 * buf, unsigned short len, uint8 * addr, unsigned short port)
{
   unsigned short ret=0;
  //发送指示灯
	 LED3(0);	
...
*/
unsigned short recvfrom(SOCKET s, uint8 * buf, unsigned short len, uint8 * addr, unsigned short *port)
{
   uint8 head[8];
   ...
   if ( len > 0 )
   {
      ...
      case Sn_MR_UDP :
        ...
        addr[2]  = head[2];
        addr[3]  = head[3];
			/*以下代码2018-10-06 21:09补充，其作用是设置UDP通信的目标IP地址*/
				if((head[0]!=W5500_Parameter_Configuration.REMOTE_Sn_DIP[0])||(head[1]!=W5500_Parameter_Configuration.REMOTE_Sn_DIP[1])||(head[2]!=W5500_Parameter_Configuration.REMOTE_Sn_DIP[2])||(head[3]!=W5500_Parameter_Configuration.REMOTE_Sn_DIP[3]))
				{
					W5500_Parameter_Configuration.REMOTE_Sn_DIP[0] = head[0];
					W5500_Parameter_Configuration.REMOTE_Sn_DIP[1] = head[1];
					W5500_Parameter_Configuration.REMOTE_Sn_DIP[2] = head[2];
					W5500_Parameter_Configuration.REMOTE_Sn_DIP[3] = head[3];
					I2C_BUFF[0] = head[0];
					I2C_BUFF[1] = head[1];
					I2C_BUFF[2] = head[2];
					I2C_BUFF[3] = head[3];
					AT24CXX_Write(5,I2C_BUFF,4); 
		      setDIPR(SOCK_UDPS,W5500_Parameter_Configuration.REMOTE_Sn_DIP); //设置目的IP	
        ...
        *port    = (*port << 8) + head[5];
				/*以下代码补充设置目标端口号*/
				if(*port != W5500_Parameter_Configuration.Sn_UDP_DPort)
				{
					W5500_Parameter_Configuration.Sn_UDP_DPort = *port;
					I2C_BUFF[0] = head[4];
					I2C_BUFF[1] = head[5];
			  	setDPORT(SOCK_UDPS,W5500_Parameter_Configuration.Sn_UDP_DPort); //设置目的端口号
      		AT24CXX_Write(27,I2C_BUFF,2); //把目的端口号写入到EEPROM
...
*/
unsigned short macraw_send( const uint8 * buf, unsigned short len )
{
   unsigned short ret=0;
   uint8 sock_num;
   sock_num =0;
...
*@return	This function return received data size for success else 0.
*/
unsigned short macraw_recv( uint8 * buf, unsigned short len )
{
   uint8 sock_num;
   unsigned short data_len=0;
   unsigned short dummyPort = 0;
